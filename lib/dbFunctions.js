...
exports.listData = function (dbInput) {
  dbInput = jsonParser.parseSpace(dbInput.slice(7))
...
exports.insertData = function (dbInput) {
  dbInput = jsonParser.parseSpace(dbInput.slice(6))
  let res = getKeyValue(dbInput)
  if (res !== 'Invalid Json' && res[1] !== '' && res[0] !== '') {
...
exports.showValue = function (dbInput) {
  dbInput = jsonParser.parseSpace(dbInput.slice(4))
  if (!fs.existsSync(db)) return 'No data available. Please insert and proceed'
  else {
    let res = getKeyValue(dbInput)
...
function getKeyValue (dbInput) {
  ...
  dbInput = jsonParser.parseSpace(dbInput.slice(key.length))
  if (dbInput) {
